...
class _DialogsIter(RequestIter):
    ...
    async def _load_next_chunk(self):
        ...
        entities = {utils.get_peer_id(
            x): x for x in itertools.chain(r.users, r.chats)}
        ...
        self.request.offset_peer = entities[utils.get_peer_id(
            r.dialogs[-1].peer)]
...
class _DraftsIter(RequestIter):
    async def _init(self, entities, **kwargs):
        ...
        self.buffer.extend(
            custom.Draft(
                self.client, entities[utils.get_peer_id(d.peer)], d.draft)
            for d in items
        )
    ...
    async def edit_folder(
        ...
        *,
        ...
        if (entity is None) == (unpack is None):
            raise ValueError(
                "You can only set either entities or unpack, not both")
        ...
        if not utils.is_list_like(folder):
            ...
            raise ValueError(
                "Number of folders does not match number of entities")
